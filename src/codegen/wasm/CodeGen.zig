...
/// Contains a list of current branches. Stores LocalPool before entering branch.
branches: std.ArrayList(LocalPool) = .empty,
...
wasm_param_count: u32 = undefined,
...
const WValue = union(enum) {
    ...
    /// Returns the offset from the bottom of the stack.
    fn memOffset(value: WValue) u64 {
        return switch (value) {
            .stack_offset => |stack_offset| stack_offset.value,
            .dead => unreachable,
            else => 0,
        };
    }
    ...
    fn toLocal(value: WValue, gen: *CodeGen, ty: Type) InnerError!WValue {
        switch (value) {
            .stack => {
                ...
                try gen.addLocal(.local_set, new_local.local);
                return new_local;
            },
            else => return value,
        }
    ...
    fn free(value: *WValue, cg: *CodeGen) void {
...
const LocalPool = struct {
    const OptionalIndex = enum(u32) {
        none = math.maxInt(u32),
        _,

        fn init(index: u32) OptionalIndex {
            const result: OptionalIndex = @fromBackingInt(index);
            assert(result != .none);
            return result;
        }

        fn unwrap(index: OptionalIndex) ?u32 {
            return if (index == .none) null else @backingInt(index);
        }
    };

    const Local = union {
        ref_count: u32,
        next_free: OptionalIndex,
    };

    locals: std.ArrayList(Local) = .empty,
    free_i32: OptionalIndex = .none,
    free_i64: OptionalIndex = .none,
    free_f32: OptionalIndex = .none,
    free_f64: OptionalIndex = .none,
    free_v128: OptionalIndex = .none,

    fn deinit(pool: *LocalPool, gpa: Allocator) void {
        pool.locals.deinit(gpa);
        pool.* = undefined;
    }

    fn clone(pool: *LocalPool, gpa: Allocator) Allocator.Error!LocalPool {
        return .{
            .locals = try pool.locals.clone(gpa),
            .free_i32 = pool.free_i32,
            .free_i64 = pool.free_i64,
            .free_f32 = pool.free_f32,
            .free_f64 = pool.free_f64,
            .free_v128 = pool.free_v128,
        };
    }

    fn freeList(pool: *LocalPool, valtype: std.wasm.Valtype) *OptionalIndex {
    ...
    fn restore(snapshot: *LocalPool, cg: *CodeGen) void {
        const local_pool = &cg.local_pool;
        const old_local_count = snapshot.locals.items.len;
        assert(local_pool.locals.items.len >= old_local_count);
        @memcpy(local_pool.locals.items[0..old_local_count], snapshot.locals.items);
        local_pool.free_i32 = snapshot.free_i32;
        local_pool.free_i64 = snapshot.free_i64;
        local_pool.free_f32 = snapshot.free_f32;
        local_pool.free_f64 = snapshot.free_f64;
        local_pool.free_v128 = snapshot.free_v128;

        for (old_local_count..local_pool.locals.items.len) |index| {
            cg.appendFreeLocal(cg.wasm_param_count + @as(u32, @intCast(index)));
        }
    }
};
...
fn finishAir(cg: *CodeGen, inst: Air.Inst.Index, result: WValue, operands: []const Air.Inst.Ref) InnerError!void {
    return cg.finishAirWithBranchDeaths(inst, result, operands, &.{});
}

/// Takes `branch_deaths` from continuing branch and calls `processDeath` after result retaining logic.
fn finishAirWithBranchDeaths(
    cg: *CodeGen,
    inst: Air.Inst.Index,
    result: WValue,
    operands: []const Air.Inst.Ref,
    branch_deaths: []const Air.Inst.Index,
) InnerError!void {
    assert(operands.len <= Air.Liveness.bpi - 1);

    if (result == .local) aliased: {
        for (operands) |operand| {
            if (operand.toIndex() == null) continue;
            const resolved_operand = cg.values.getPtr(operand) orelse unreachable;
            if (resolved_operand.* != .local or resolved_operand.local != result.local) continue;

            if (result.local >= cg.wasm_param_count) {
                const local = &cg.local_pool.locals.items[result.local - cg.wasm_param_count];
                assert(local.ref_count > 0);
                local.ref_count += 1;
            }
            break :aliased;
        }
    ...
    for (branch_deaths) |death| cg.processDeath(death.toRef());
...
fn pushBranch(cg: *CodeGen) InnerError!void {
    var snapshot = try cg.local_pool.clone(cg.gpa);
    errdefer snapshot.deinit(cg.gpa);
    try cg.branches.append(cg.gpa, snapshot);
}

fn popBranch(cg: *CodeGen) void {
    var snapshot = cg.branches.pop().?;
    defer snapshot.deinit(cg.gpa);
    snapshot.restore(cg);
}
...
fn offsetPointer(cg: *CodeGen, ptr: WValue, offset: u64) InnerError!WValue {
    if (offset == 0) return ptr;
    return switch (ptr) {
        .stack_offset => |stack_offset| .{ .stack_offset = .{
            .value = stack_offset.value + offset,
        } },
        else => result: {
            try cg.emitWValue(ptr);
            switch (cg.ptr_size) {
                .wasm32 => {
                    try cg.addImm32(@intCast(offset));
                    try cg.addTag(.i32_add);
                },
                .wasm64 => {
                    try cg.addImm64(offset);
...
fn emitWValue(cg: *CodeGen, value: WValue) InnerError!void {
    switch (value) {
        ...
        .none, .stack => {}, // no-op
        .local => |idx| try cg.addLocal(.local_get, idx),
        ...
        .stack_offset => |stack_offset| {
            ...
            _ = try cg.offsetPointer(.stack, stack_offset.value);
        },
    }
}

/// Emits the base address for a memory instruction.
fn emitMemBase(cg: *CodeGen, ptr: WValue) InnerError!void {
    switch (ptr) {
        .stack_offset => try cg.addLocal(.local_get, cg.bottom_stack_value.local),
        else => try cg.emitWValue(ptr),
    }
}

fn appendFreeLocal(cg: *CodeGen, index: u32) void {
    assert(index >= cg.wasm_param_count);
    const mir_index = index - cg.wasm_param_count;
    const valtype = cg.mir_locals.items[mir_index];
    const free_list = cg.local_pool.freeList(valtype);
    cg.local_pool.locals.items[mir_index] = .{ .next_free = free_list.* };
    free_list.* = .init(index);
    log.debug("freed local ({d}) of type {}", .{ index, valtype });
}

fn releaseLocal(cg: *CodeGen, index: u32) void {
    if (index < cg.wasm_param_count) return;
    const local = &cg.local_pool.locals.items[index - cg.wasm_param_count];
    assert(local.ref_count > 0);
    local.ref_count -= 1;
    if (local.ref_count > 0) return;
    cg.appendFreeLocal(index);
}
...
fn allocLocal(cg: *CodeGen, ty: Type) InnerError!WValue {
    ...
    const free_list = cg.local_pool.freeList(valtype);
    if (free_list.unwrap()) |index| {
        const local = &cg.local_pool.locals.items[index - cg.wasm_param_count];
        free_list.* = local.next_free;
        local.* = .{ .ref_count = 1 };
...
fn ensureAllocLocal(cg: *CodeGen, ty: Type) InnerError!WValue {
    const zcu = cg.pt.zcu;
    try cg.mir_locals.ensureUnusedCapacity(cg.gpa, 1);
    try cg.local_pool.locals.ensureUnusedCapacity(cg.gpa, 1);
    assert(cg.mir_locals.items.len == cg.local_pool.locals.items.len);
    const initial_index = cg.wasm_param_count + @as(u32, @intCast(cg.local_pool.locals.items.len));
    cg.mir_locals.appendAssumeCapacity(typeToValtype(ty, zcu, cg.target));
    cg.local_pool.locals.appendAssumeCapacity(.{ .ref_count = 1 });
    return .{ .local = initial_index };
}
...
fn generateInner(cg: *CodeGen, any_returns: bool) InnerError!Mir {
    const zcu = cg.pt.zcu;
    cg.wasm_param_count = @intCast(cg.args.len + @intFromBool(cg.return_value != .none) + @intFromBool(cg.varargs != .none));
    cg.initial_stack_value = try cg.ensureAllocLocal(Type.usize);
    cg.bottom_stack_value = try cg.ensureAllocLocal(Type.usize);
    defer assert(cg.branches.items.len == 0);
    ...
    return .{
        ...
        .prologue = .{
            .sp_local = cg.initial_stack_value.local,
            .flags = .{ .stack_alignment = cg.stack_alignment },
            .stack_size = cg.stack_size,
            .bottom_stack_local = cg.bottom_stack_value.local,
        },
...
fn resolveCallingConventionValues(
    ...
    if (firstParamSRet(fn_info.cc, Type.fromInterned(fn_info.return_type), zcu, target)) {
        ...
        result.return_value = .{ .local = result.local_index };
    ...
    switch (cc) {
        .auto => {
            for (fn_info.param_types.get(ip)) |ty| {
                ...
                try args.append(.{ .local = result.local_index });
                result.local_index += 1;
            }
        },
        .wasm_mvp => {
            for (fn_info.param_types.get(ip)) |ty| {
                ...
                switch (abi.classifyType(param_ty, zcu, target)) {
                    ...
                    .double_i64 => {
                        try args.append(.{ .local = result.local_index });
                        try args.append(.{ .local = result.local_index + 1 });
    ...
    if (fn_info.is_var_args) {
        result.varargs = .{ .local = result.local_index };
        result.local_index += 1;
    }
...
fn lowerArg(cg: *CodeGen, cc: std.lang.CallingConvention, ty: Type, value: WValue) !void {
    ...
    switch (abi.classifyType(ty, zcu, cg.target)) {
        ...
        .double_i64 => {
            ...
            const base_offset = value.memOffset();
            try cg.emitMemBase(value);
            try cg.addMemArg(.i64_load, .{ .offset = base_offset, .alignment = .@"8" });
            try cg.emitMemBase(value);
            try cg.addMemArg(.i64_load, .{ .offset = base_offset + 8, .alignment = .@"8" });
        },
        .indirect => {
            const stack_copy = try cg.allocStack(ty);
            try cg.store(stack_copy, value, ty, .{});
            return cg.emitWValue(stack_copy);
        },
        .unrolled => |vector| {
...
fn allocStackBytes(cg: *CodeGen, size: u32, alignment: Alignment) !WValue {
    ...
    return .{ .stack_offset = .{ .value = offset } };
}
...
fn airVaArg(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    try cg.addLocal(.local_set, arg_ptr.local);

    try cg.emitWValue(operand);
    try cg.emitWValue(arg_ptr);
    switch (cg.ptr_size) {
        .wasm32 => {
...
fn store(cg: *CodeGen, lhs: WValue, rhs: WValue, ty: Type, memarg: MemArg) InnerError!void {
    ...
    if (isByRef(ty, zcu, cg.target)) {
        const offset_ptr = try cg.offsetPointer(lhs, memarg.offset);
        return cg.memcpy(offset_ptr, rhs, .{ .imm32 = @intCast(abi_size) });
    }

    const base_offset = lhs.memOffset();
    try cg.emitMemBase(lhs);
...
fn load(cg: *CodeGen, operand: WValue, ty: Type, memarg: MemArg) InnerError!WValue {
    ...
    if (isByRef(ty, zcu, cg.target)) {
        var src_ptr = try cg.offsetPointer(operand, memarg.offset);
        const owns_src_ptr = src_ptr == .stack;
        src_ptr = try src_ptr.toLocal(cg, .usize);
        defer if (owns_src_ptr) src_ptr.free(cg);

        const new_ptr = try cg.allocStack(ty);
        try cg.memcpy(new_ptr, src_ptr, .{ .imm32 = @intCast(ty.abiSize(zcu)) });
        return new_ptr;
    }

    const base_offset = operand.memOffset();
...
fn intNot(cg: *CodeGen, ty: IntType, operand: WValue) InnerError!WValue {
    switch (ty.bits) {
        ...
        65...128 => {
            ...
            const result_offset = result.memOffset();
            try cg.emitMemBase(result);
            _ = try cg.load(operand, .u64, .{});
            try cg.addImm64(~@as(u64, 0));
            try cg.addTag(.i64_xor);
            try cg.store(.stack, .stack, .u64, .{ .offset = result_offset });
            ...
            try cg.addTag(.i64_xor);
            try cg.store(.stack, .stack, .u64, .{ .offset = result_offset + 8 });
...
fn intAbs(cg: *CodeGen, ty: IntType, operand: WValue) InnerError!WValue {
    ...
    switch (ty.bits) {
        ...
        1...32 => {
            ...
            defer mask.free(cg);
            try cg.addLocal(.local_tee, mask.local);
        ...
        65...128 => {
            ...
            const mask_offset = mask.memOffset();
            try cg.emitMemBase(mask);
            try cg.emitMemBase(mask);
            ...
            defer tmp.free(cg);
            try cg.addLocal(.local_tee, tmp.local);
            try cg.store(.stack, .stack, .u64, .{ .offset = mask_offset });
            try cg.emitWValue(tmp);
            try cg.store(.stack, .stack, .u64, .{ .offset = mask_offset + 8 });
...
fn intBitReverse(cg: *CodeGen, ty: IntType, operand: WValue) InnerError!WValue {
    switch (ty.bits) {
        ...
        65...128 => {
            ...
            const tmp_offset = tmp.memOffset();
            try cg.emitMemBase(tmp);
            ...
            try cg.store(.stack, .stack, .u64, .{ .offset = tmp_offset });
            ...
            try cg.store(.stack, .stack, .u64, .{ .offset = tmp_offset + 8 });
...
fn intByteSwap(cg: *CodeGen, ty: IntType, operand: WValue) InnerError!WValue {
    switch (ty.bits) {
        ...
        65...128 => {
            ...
            try cg.store(.stack, swap_low, .u64, .{ .offset = result_offset + 8 });
            ...
            try cg.store(.stack, swap_high, .u64, .{ .offset = result_offset });
    ...
    switch (ty.bits) {
        ...
        128 => return operand,
            ...
            if (ty.bits % 64 != 0) {
                ...
                try cg.store(.stack, .stack, .u64, .{ .offset = result.memOffset() + used_len - 8 });
            } else {
            ...
            if (used_len + 8 == full_len) { // last limb needs sign extended
                ...
                try cg.store(.stack, .stack, .u64, .{ .offset = result.memOffset() + used_len });
            }
...
fn toLocalInt(cg: *CodeGen, value: WValue, int_ty: IntType) InnerError!WValue {
    switch (value) {
        .stack => {
            ...
            try cg.addLocal(.local_set, new_local.local);
            return new_local;
        },
    ...
    if (src_bits == 64 and dest_bits == 32) {
        ...
        if (dest_bits <= src_bits) {
            ...
            if (src_bits == 32) {
                ...
                try cg.emitWValue(operand);
                try cg.addTag(if (src_ty.is_signed) .i64_extend_i32_s else .i64_extend_i32_u);
                try cg.store(.stack, .stack, .u64, .{ .offset = result.memOffset() });
                src_len = 8;
            } else if (src_bits == 64) {
                try cg.emitMemBase(result);
                try cg.emitWValue(operand);
            ...
            if (dest_bits == 128) {
                if (src_ty.is_signed) {
                    ...
                    try cg.store(.stack, shr, .u64, .{ .offset = result.memOffset() + 8 });
                } else {
                    try cg.store(result, .{ .imm64 = 0 }, .u64, .{ .offset = 8 });
                }
            } else {
...
fn floatNeg(cg: *CodeGen, ty: FloatType, arg: WValue) InnerError!WValue {
    switch (ty) {
        ...
        .f80, .f128 => {
            ...
            const result_offset = result.memOffset();
            const arg_offset = arg.memOffset();
            try cg.emitMemBase(result);
            try cg.emitMemBase(arg);
            try cg.addMemArg(.i64_load, .{ .offset = arg_offset, .alignment = .@"2" });
            try cg.addMemArg(.i64_store, .{ .offset = result_offset, .alignment = .@"2" });
            try cg.emitMemBase(result);
            try cg.emitMemBase(arg);
            try cg.addMemArg(.i64_load, .{ .offset = arg_offset + 8, .alignment = .@"2" });
            if (ty == .f80) {
            ...
            try cg.addTag(.i64_xor);
            try cg.addMemArg(.i64_store, .{ .offset = result_offset + 8, .alignment = .@"2" });
            return result;
        },
    }
}
...
fn lowerBlock(cg: *CodeGen, inst: Air.Inst.Index, block_ty: Type, body: []const Air.Inst.Index) InnerError!void {
    ...
    defer assert(cg.blocks.swapRemove(inst));
    ...
    for (cg.liveness.getBlock(inst).deaths) |death| cg.processDeath(death.toRef());
...
fn airCondBr(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    const branch_liveness = cg.liveness.getCondBr(inst);
    ...
    if (cg.liveness.operandDies(inst, 0)) cg.processDeath(cond_br.condition);

    {
        try cg.pushBranch();
        defer cg.popBranch();
        for (branch_liveness.else_deaths) |death| cg.processDeath(death.toRef());
    ...
    // Outer block that matches the condition
    {
        try cg.pushBranch();
        defer cg.popBranch();
        for (branch_liveness.then_deaths) |death| cg.processDeath(death.toRef());
...
fn airCmp(cg: *CodeGen, inst: Air.Inst.Index, op: std.math.CompareOperator) InnerError!void {
    ...
    if (type_tag == .optional and !operand_ty.optionalReprIsPayload(zcu)) {
        ...
        if (payload_ty.hasRuntimeBits(zcu)) {
            ...
            try cg.addLocal(.local_set, result.local);

            _ = try cg.isNull(lhs, operand_ty, .i32_eq, .value);
            try cg.addLocal(.local_tee, lhs_null.local);
            ...
            try cg.addLocal(.local_get, lhs_null.local);
            try cg.addLabel(.br_if, 0);
            ...
            try cg.addLocal(.local_get, result.local);
...
fn airIntFromPtr(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    if (operand == .stack_offset) {
        // materialize operand, we assume precomputed value is more efficient
        try cg.emitWValue(operand);
...
fn structFieldPtr(
    ...
    return cg.offsetPointer(struct_ptr, offset);
}
...
fn airSwitchBr(cg: *CodeGen, inst: Air.Inst.Index, is_dispatch_loop: bool) InnerError!void {
    ...
    const branch_liveness = try cg.liveness.getSwitchBr(cg.gpa, inst, switch_br.cases_len + 1);
    defer cg.gpa.free(branch_liveness.deaths);
    ...
    const target = if (is_dispatch_loop) target: {
        ...
        try cg.emitWValue(initial_target);
        try cg.addLocal(.local_set, owned_target.local);
    ...
    defer if (is_dispatch_loop) assert(cg.blocks.swapRemove(inst));
    ...
    if (switch_br.cases_len == 0) {
...

        {
            try cg.pushBranch();
            defer cg.popBranch();
            for (branch_liveness.deaths[0]) |death| cg.processDeath(death.toRef());
            try cg.genBody(else_body);
        }
    ...
    if (has_else_body) {
...

        {
            try cg.pushBranch();
            defer cg.popBranch();
            for (branch_liveness.deaths[branch_liveness.deaths.len - 1]) |death| cg.processDeath(death.toRef());
            try cg.genBody(else_body);
        }
    } else {
        try cg.addTag(.@"unreachable");
...
fn airSwitchDispatch(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    const operand = try cg.resolveInst(br.operand);
    try cg.emitWValue(operand);
    try cg.addLocal(.local_set, switch_loop.value.local);
...
fn airUnwrapErrUnionPayload(cg: *CodeGen, inst: Air.Inst.Index, op_is_ptr: bool) InnerError!void {
    ...
    const result: WValue = result: {
        ...
        const pl_offset = errUnionPayloadOffset(payload_ty, zcu);
        if (op_is_ptr or isByRef(payload_ty, zcu, cg.target)) {
            break :result try cg.offsetPointer(operand, pl_offset);
        } else {
...
fn airWrapErrUnionPayload(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    const result = result: {
        ...
        try cg.store(err_union, operand, pl_ty, .{ .offset = errUnionPayloadOffset(pl_ty, zcu) });

        // ensure we also write '0' to the error part, so any present stack value gets overwritten by it.
        try cg.emitMemBase(err_union);
        try cg.addImm32(0);
        const err_val_offset = errUnionErrorOffset(pl_ty, zcu);
        try cg.addMemArg(.i32_store16, .{
            .offset = err_union.memOffset() + err_val_offset,
            .alignment = .@"2",
        });
...
fn airWrapErrUnionErr(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    const result = result: {
        ...
        const payload_ptr = try cg.offsetPointer(err_union, errUnionPayloadOffset(pl_ty, zcu));
...
fn isNull(cg: *CodeGen, operand: WValue, op_ty: Type, opcode: std.wasm.Opcode, op_kind: OpKind) InnerError!WValue {
    ...
    if (!optional_ty.optionalReprIsPayload(zcu)) {
        ...
        try cg.emitMemBase(operand);
        switch (cg.ptr_size) {
            .wasm32 => try cg.addMemArg(.i32_load, .{ .offset = base_offset, .alignment = .@"4" }),
            .wasm64 => try cg.addMemArg(.i64_load, .{ .offset = base_offset, .alignment = .@"8" }),
        }
    } else {
        if (op_kind == .ptr) {
            try cg.emitMemBase(operand);
...
fn airOptionalPayloadPtrSet(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    try cg.emitMemBase(operand);
    try cg.addImm32(1);
    try cg.addMemArg(.i32_store8, .{ .offset = operand.memOffset() + offset, .alignment = .@"1" });
...
fn airWrapOptional(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    const result = result: {
        if (!payload_ty.hasRuntimeBits(zcu)) {
            ...
            try cg.emitMemBase(non_null_bit);
            try cg.addImm32(1);
            try cg.addMemArg(.i32_store8, .{ .offset = non_null_bit.memOffset(), .alignment = .@"1" });
        ...
        try cg.emitMemBase(result_ptr);
        try cg.addImm32(1);
        try cg.addMemArg(.i32_store8, .{ .offset = result_ptr.memOffset() + offset, .alignment = .@"1" });

        try cg.store(result_ptr, operand, payload_ty, .{});
...
fn memset(cg: *CodeGen, elem_ty: Type, ptr: WValue, len: WValue, value: WValue) InnerError!void {
    ...
    var new_ptr = try cg.allocLocal(Type.usize);
    defer new_ptr.free(cg);
    try cg.emitWValue(ptr);
    try cg.addLocal(.local_set, new_ptr.local);
    ...
    try cg.addLocal(.local_set, end_ptr.local);
...
fn airShuffleOne(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    for (mask, 0..) |mask_elem, out_idx| {
        ...
        try cg.store(.stack, elem_val, elem_ty, .{ .offset = dest_alloc.memOffset() + elem_size * out_idx });
    }
            ...
            .@"struct" => switch (result_ty.containerLayout(zcu)) {
                .@"packed" => unreachable, // legalize .expand_packed_aggregate_init
                    ...
                    for (elements, 0..) |elem, elem_index| {
                        ...
                        try cg.store(result, value, elem_ty, .{ .offset = field_offset });
                    }
...
fn airErrUnionPayloadPtrSet(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    const result = try cg.offsetPointer(operand, errUnionPayloadOffset(payload_ty, zcu));
    return cg.finishAir(inst, result, &.{ty_op.operand});
}
...
fn airFieldParentPtr(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    if (field_offset == 0) {
        return cg.finishAir(inst, field_ptr, &.{extra.field_ptr});
    }

    // maybe use offsetPointer? sub looks better in disasm
    try cg.emitWValue(field_ptr);
    switch (cg.ptr_size) {
        .wasm32 => {
        ...
        .wasm64 => {
            try cg.addImm64(field_offset);
            try cg.addTag(.i64_sub);
        },
    }
    return cg.finishAir(inst, .stack, &.{extra.field_ptr});
}
    ...
    var len = switch (dst_ty.ptrSize(zcu)) {
        .slice => blk: {
            ...
            if (ptr_elem_ty.abiSize(zcu) != 1) {
                ...
                try cg.addTag(.i32_mul);
                try cg.addLocal(.local_set, slice_len.local);
            }
            break :blk slice_len;
        },
        .one => @as(WValue, .{
...
fn airPtrSliceFieldPtr(cg: *CodeGen, inst: Air.Inst.Index, offset: u32) InnerError!void {
    ...
    const result = try cg.offsetPointer(slice_ptr, offset);
    return cg.finishAir(inst, result, &.{ty_op.operand});
}
...
fn airTry(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    const result = try lowerTry(cg, inst, unwrapped_try.error_union, err_union, body, err_union_ty, false);
    return cg.finishAirWithBranchDeaths(
        inst,
        result,
        &.{unwrapped_try.error_union},
        cg.liveness.getCondBr(inst).then_deaths,
    );
}
...
fn airTryPtr(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    const result = try lowerTry(cg, inst, unwrapped_try.error_union_ptr, err_union_ptr, body, err_union_ty, true);
    return cg.finishAirWithBranchDeaths(
        inst,
        result,
        &.{unwrapped_try.error_union_ptr},
        cg.liveness.getCondBr(inst).then_deaths,
    );
}
...
fn lowerTry(
    ...
    if (!err_union_ty.errorUnionSet(zcu).errorSetIsEmpty(zcu)) {
...

        {
            try cg.pushBranch();
            defer cg.popBranch();
            if (cg.liveness.operandDies(inst, 0)) cg.processDeath(operand_ref);
            for (branch_liveness.else_deaths) |death| cg.processDeath(death.toRef());
            try cg.genBody(body);
        }
        try cg.endBlock();
    ...
    const pl_offset = errUnionPayloadOffset(pl_ty, zcu);
    if (operand_is_ptr or isByRef(pl_ty, zcu, cg.target)) {
        return cg.offsetPointer(err_union, pl_offset);
    }
    return cg.load(err_union, pl_ty, .{ .offset = pl_offset });
}
...
fn airCmpxchg(cg: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    var ptr_val = if (cg.useAtomicFeature()) val: {
        ...
        try cg.addLocal(.local_set, cmp_result.local);
        break :val val_local;
    } else val: {
        if (ty.abiSize(zcu) > 8) {
        ...
        try cg.addLocal(.local_tee, cmp_result.local);
        try cg.addTag(.select);
        try cg.store(.stack, .stack, ty, .{ .offset = ptr_operand.memOffset() });
    ...
    if (cg.useAtomicFeature()) {
        ...
        switch (op) {
            ...
            => {
                ...
                try cg.addLocal(.local_set, value.local);
        ...
        switch (op) {
            ...
            => {
                if (ty.isAnyFloat()) {
                    ...
                    try cg.store(.stack, .stack, ty, .{ .offset = ptr.memOffset() });
                } else {
                    const int_ty: IntType = .fromType(cg, ty);
                    try cg.emitMemBase(ptr);
                    _ = switch (op) {
    ...
    if (unwrapped_asm.source.len != 0) {
...

        {
            ...
            if (it.next()) |output| {
                ...
                const gop = try local_map.getOrPutValue(cg.gpa, name, result.local);
