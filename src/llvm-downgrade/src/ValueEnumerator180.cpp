//===- ValueEnumerator180.cpp - Number values and types for bitcode writer ---===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the ValueEnumerator180 class.
//
//===----------------------------------------------------------------------===//
...
static OrderMap orderModule(const Module &M) {
  // This needs to match the order used by ValueEnumerator180::ValueEnumerator180()
  // and ValueEnumerator180::incorporateFunction().
  OrderMap OM;

  // Initializers of GlobalValues are processed in
  // BitcodeReader::ResolveGlobalAndAliasInits().  Match the order there rather
  // than ValueEnumerator180, and match the code in predictValueUseListOrderImpl()
  ...
  for (const Function &F : M) {
    if (F.isDeclaration())
      continue;
    // Here we need to match the union of ValueEnumerator180::incorporateFunction()
...
ValueEnumerator180::ValueEnumerator180(const Module &M,
  ...
  //
  // TODO: Move this to ValueEnumerator180::EnumerateOperandType() once bitcode
...
unsigned ValueEnumerator180::getInstructionID(const Instruction *Inst) const {
  InstructionMapType::const_iterator I = InstructionMap.find(Inst);
  assert(I != InstructionMap.end() && "Instruction is not mapped!");
  return I->second;
}

unsigned ValueEnumerator180::getComdatID(const Comdat *C) const {
  unsigned ComdatID = Comdats.idFor(C);
  assert(ComdatID && "Comdat not found!");
  return ComdatID;
}

void ValueEnumerator180::setInstructionID(const Instruction *I) {
  InstructionMap[I] = InstructionCount++;
}

unsigned ValueEnumerator180::getValueID(const Value *V) const {
...
LLVM_DUMP_METHOD void ValueEnumerator180::dump() const {
...
void ValueEnumerator180::print(raw_ostream &OS, const ValueMapType &Map,
...
void ValueEnumerator180::print(raw_ostream &OS, const MetadataMapType &Map,
                            const char *Name) const {
  OS << "Map Name: " << Name << "\n";
  OS << "Size: " << Map.size() << "\n";
  for (const auto &I : Map) {
...
void ValueEnumerator180::OptimizeConstants(unsigned CstStart, unsigned CstEnd) {
...
void ValueEnumerator180::EnumerateValueSymbolTable(const ValueSymbolTable &VST) {
...
/// module.
void ValueEnumerator180::EnumerateNamedMetadata(const Module &M) {
  for (const auto &I : M.named_metadata())
    EnumerateNamedMDNode(&I);
}

void ValueEnumerator180::EnumerateNamedMDNode(const NamedMDNode *MD) {
  for (unsigned i = 0, e = MD->getNumOperands(); i != e; ++i)
    EnumerateMetadata(nullptr, MD->getOperand(i));
}

unsigned ValueEnumerator180::getMetadataFunctionID(const Function *F) const {
  return F ? getValueID(F) + 1 : 0;
}

void ValueEnumerator180::EnumerateMetadata(const Function *F, const Metadata *MD) {
  EnumerateMetadata(getMetadataFunctionID(F), MD);
}

void ValueEnumerator180::EnumerateFunctionLocalMetadata(
    const Function &F, const LocalAsMetadata *Local) {
  EnumerateFunctionLocalMetadata(getMetadataFunctionID(&F), Local);
}

void ValueEnumerator180::EnumerateFunctionLocalListMetadata(
    const Function &F, const DIArgList *ArgList) {
  EnumerateFunctionLocalListMetadata(getMetadataFunctionID(&F), ArgList);
}

void ValueEnumerator180::dropFunctionFromMetadata(
...
void ValueEnumerator180::EnumerateMetadata(unsigned F, const Metadata *MD) {
...
const MDNode *ValueEnumerator180::enumerateMetadataImpl(unsigned F, const Metadata *MD) {
  if (!MD)
    return nullptr;
...
void ValueEnumerator180::organizeMetadata() {
...
void ValueEnumerator180::incorporateFunctionMetadata(const Function &F) {
...
void ValueEnumerator180::EnumerateValue(const Value *V) {
...
void ValueEnumerator180::EnumerateType(Type *Ty) {
...
void ValueEnumerator180::EnumerateOperandType(const Value *V) {
  EnumerateType(V->getType());
...
void ValueEnumerator180::incorporateFunction(const Function &F) {
  InstructionCount = 0;
...
void ValueEnumerator180::purgeFunction() {
...
unsigned ValueEnumerator180::getGlobalBasicBlockID(const BasicBlock *BB) const {
...
uint64_t ValueEnumerator180::computeBitsRequiredForTypeIndicies() const {
