//===- ValueEnumerator140.cpp - Number values and types for bitcode writer ---===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the ValueEnumerator140 class.
//
//===----------------------------------------------------------------------===//
...
static OrderMap orderModule(const Module &M) {
  // This needs to match the order used by ValueEnumerator140::ValueEnumerator140()
  // and ValueEnumerator140::incorporateFunction().
  OrderMap OM;
  ...
  // than ValueEnumerator140, and match the code in predictValueUseListOrderImpl()
  ...
  for (const Function &F : M) {
    if (F.isDeclaration())
      continue;
    // Here we need to match the union of ValueEnumerator140::incorporateFunction()
...
ValueEnumerator140::ValueEnumerator140(const Module &M,
                                 bool ShouldPreserveUseListOrder)
    : ShouldPreserveUseListOrder(ShouldPreserveUseListOrder) {
  if (ShouldPreserveUseListOrder)
    UseListOrders = predictUseListOrder(M);

  // Special types are needed when the context is opaque: bare opaque pointers
  // are encoded as {}* (see writeTypeTable), so ensure the {} type is available.
  {
  ...
  //
  // TODO: Move this to ValueEnumerator140::EnumerateOperandType() once bitcode
...
unsigned ValueEnumerator140::getInstructionID(const Instruction *Inst) const {
  InstructionMapType::const_iterator I = InstructionMap.find(Inst);
  assert(I != InstructionMap.end() && "Instruction is not mapped!");
  return I->second;
}

unsigned ValueEnumerator140::getComdatID(const Comdat *C) const {
  unsigned ComdatID = Comdats.idFor(C);
  assert(ComdatID && "Comdat not found!");
  return ComdatID;
}

void ValueEnumerator140::setInstructionID(const Instruction *I) {
  InstructionMap[I] = InstructionCount++;
}

unsigned ValueEnumerator140::getValueID(const Value *V) const {
...
LLVM_DUMP_METHOD void ValueEnumerator140::dump() const {
...
void ValueEnumerator140::print(raw_ostream &OS, const ValueMapType &Map,
...
void ValueEnumerator140::print(raw_ostream &OS, const MetadataMapType &Map,
                            const char *Name) const {
  OS << "Map Name: " << Name << "\n";
  OS << "Size: " << Map.size() << "\n";
  for (const auto &I : Map) {
...
void ValueEnumerator140::OptimizeConstants(unsigned CstStart, unsigned CstEnd) {
...
void ValueEnumerator140::EnumerateValueSymbolTable(const ValueSymbolTable &VST) {
...
/// module.
void ValueEnumerator140::EnumerateNamedMetadata(const Module &M) {
  for (const auto &I : M.named_metadata())
    EnumerateNamedMDNode(&I);
}

void ValueEnumerator140::EnumerateNamedMDNode(const NamedMDNode *MD) {
  for (unsigned i = 0, e = MD->getNumOperands(); i != e; ++i)
    EnumerateMetadata(nullptr, MD->getOperand(i));
}

unsigned ValueEnumerator140::getMetadataFunctionID(const Function *F) const {
  return F ? getValueID(F) + 1 : 0;
}

void ValueEnumerator140::EnumerateMetadata(const Function *F, const Metadata *MD) {
  EnumerateMetadata(getMetadataFunctionID(F), MD);
}

void ValueEnumerator140::EnumerateFunctionLocalMetadata(
    const Function &F, const LocalAsMetadata *Local) {
  EnumerateFunctionLocalMetadata(getMetadataFunctionID(&F), Local);
}

void ValueEnumerator140::EnumerateFunctionLocalListMetadata(
    const Function &F, const DIArgList *ArgList) {
  EnumerateFunctionLocalListMetadata(getMetadataFunctionID(&F), ArgList);
}

void ValueEnumerator140::dropFunctionFromMetadata(
...
void ValueEnumerator140::EnumerateMetadata(unsigned F, const Metadata *MD) {
...
const MDNode *ValueEnumerator140::enumerateMetadataImpl(unsigned F, const Metadata *MD) {
  if (!MD)
    return nullptr;
...
void ValueEnumerator140::organizeMetadata() {
...
void ValueEnumerator140::incorporateFunctionMetadata(const Function &F) {
...
void ValueEnumerator140::EnumerateValue(const Value *V) {
...
void ValueEnumerator140::EnumerateType(Type *Ty) {
...
void ValueEnumerator140::EnumerateOperandType(const Value *V) {
  EnumerateType(V->getType());
...
// Defined in BitcodeWriter140.cpp; counts how many attribute entries the
// writer will emit for AS, so unencodable groups can be dropped consistently.
extern unsigned countEncodableAttrs140(const AttributeSet &AS);

void ValueEnumerator140::EnumerateAttributes(AttributeList PAL) {
  ...
  for (unsigned i : PAL.indexes()) {
    ...
    // record (which the LLVM 14 reader rejects), e.g. a lone memory(readwrite)
    // or captures(...) that decomposes to nothing. countEncodableAttrs140 is
    // the counting mode of the writer's own encoding function, so the two
    // cannot drift apart.
    auto AS_index = i;
    if (countEncodableAttrs140(AS) == 0) {
...
void ValueEnumerator140::incorporateFunction(const Function &F) {
  InstructionCount = 0;
...
void ValueEnumerator140::purgeFunction() {
...
unsigned ValueEnumerator140::getGlobalBasicBlockID(const BasicBlock *BB) const {
...
uint64_t ValueEnumerator140::computeBitsRequiredForTypeIndicies() const {
