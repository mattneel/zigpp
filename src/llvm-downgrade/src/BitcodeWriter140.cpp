//===- Bitcode/Writer/BitcodeWriter140.cpp - Bitcode Writer ------------------===//
//
...
public:
  /// Constructs a BitcodeWriterBase140 object that writes to the provided
  /// \p Stream.
  BitcodeWriterBase140(BitstreamWriter &Stream, StringTableBuilder &StrtabBuilder)
...
void BitcodeWriterBase140::writeModuleVersion() {
  // VERSION: [version#]
...
/// ModuleBitcodeWriter140 and ThinLinkBitcodeWriter.
class ModuleBitcodeWriterBase140 : public BitcodeWriterBase140 {
protected:
...
public:
  /// Constructs a ModuleBitcodeWriterBase140 object for the given Module,
  /// writing to the provided \p Buffer.
  ModuleBitcodeWriterBase140(const Module &M, StringTableBuilder &StrtabBuilder,
                          BitstreamWriter &Stream,
    ...
    // ValueEnumerator140, so that they can be emitted in the VST.
...
class ModuleBitcodeWriter140 : public ModuleBitcodeWriterBase140 {
...
public:
  /// Constructs a ModuleBitcodeWriter140 object for the given Module,
  /// writing to the provided \p Buffer.
  ModuleBitcodeWriter140(const Module &M, SmallVectorImpl<char> &Buffer,
                      StringTableBuilder &StrtabBuilder,
                      BitstreamWriter &Stream, bool ShouldPreserveUseListOrder,
                      const ModuleSummaryIndex *Index, bool GenerateHash,
                      ModuleHash *ModHash = nullptr)
      : ModuleBitcodeWriterBase140(M, StrtabBuilder, Stream,
...
class IndexBitcodeWriter140 : public BitcodeWriterBase140 {
...
public:
  /// Constructs a IndexBitcodeWriter140 object for the given combined index,
  /// writing to the provided \p Buffer. When writing a subset of the index
  /// for a distributed backend, provide a \p ModuleToSummariesForIndex map.
  IndexBitcodeWriter140(BitstreamWriter &Stream, StringTableBuilder &StrtabBuilder,
                     const ModuleSummaryIndex &Index,
                     const std::map<std::string, GVSummaryMapTy>
                         *ModuleToSummariesForIndex = nullptr)
      : BitcodeWriterBase140(Stream, StrtabBuilder), Index(Index),
...
// Returns ATTR_KIND_INVALID for attribute kinds with no LLVM 14 encoding
// (those introduced later); the caller drops them. uwtable/memory/captures are
// handled by encodeAttribute140 before this table is consulted, since their
// modern representations need translation rather than a 1:1 code mapping.
static uint64_t getAttrKindEncoding140(Attribute::AttrKind Kind) {
  switch (Kind) {
  default:
...
// Append the LLVM 14 encoding of Attr to Record, returning how many attribute
// entries were emitted. A null Record only counts (VE may then be null too).
// ValueEnumerator140 uses the counting mode (via countEncodableAttrs140) to
...
// (which the LLVM 14 reader rejects).
unsigned encodeAttribute140(const Attribute &Attr,
                            SmallVectorImpl<uint64_t> *Record,
                            const ValueEnumerator140 *VE) {
  auto Emit = [&](std::initializer_list<uint64_t> Vals) {
    if (Record)
      Record->append(Vals);
  };

  // LLVM 14 has no memory(...) attribute; decompose the MemoryEffects into
  // the legacy standalone enum attributes the LLVM 14 reader knows.
  ...
  // LLVM 14 has no captures(...) attribute; it only knows the legacy
  ...
  // uwtable became an int attribute (sync/async) in LLVM 15; emitting it as
  // one produces bitcode LLVM 14 rejects ("Not an int attribute"). Both kinds
  // ask codegen for unwind tables, which is also what LLVM 14's plain enum
  // uwtable does, so emit that.
  ...
  if (Attr.isEnumAttribute() || Attr.isIntAttribute()) {
    // Attribute kinds introduced after LLVM 14 (nofpclass, allockind,
    // dead_on_unwind, nocreateundeforpoison, ...) have no LLVM 14 encoding;
    // they are optimization hints, so drop them.
    const uint64_t Enc = getAttrKindEncoding140(Attr.getKindAsEnum());
  ...
  // have no LLVM 14 encoding -- and Metal's AIR loader rejects them; drop.
  return 0;
}

unsigned countEncodableAttrs140(const AttributeSet &AS) {
  unsigned N = 0;
  for (Attribute Attr : AS)
    N += encodeAttribute140(Attr, nullptr, nullptr);
  return N;
}

void ModuleBitcodeWriter140::writeAttributeGroupTable() {
  const std::vector<ValueEnumerator140::IndexAndAttrSet> &AttrGrps =
  ...
  for (ValueEnumerator140::IndexAndAttrSet Pair : AttrGrps) {
    if (Pair.first == ValueEnumerator140::invalid_attribute_group_id) {
      // this complete set/group can't be encoded for 14.0
      continue;
    }
    ...
    for (Attribute Attr : AS)
      encodeAttribute140(Attr, &Record, &VE);
...
void ModuleBitcodeWriter140::writeAttributeTable() {
    ...
    for (unsigned i : AL.indexes()) {
      ...
      if (AS.hasAttributes())
        if (const auto group_id = VE.getAttributeGroupID({i, AS});
            group_id != ValueEnumerator140::invalid_attribute_group_id)
...
void ModuleBitcodeWriter140::writeTypeTable() {
  const ValueEnumerator140::TypeList &TypeList = VE.getTypes();
  ...
  for (Type *T : TypeList) {
    ...
    case Type::PointerTyID: {
      ...
      // The consumers of this LLVM 14 bitcode (e.g. Metal's AIR loader) do not
      // support opaque pointers, so never emit TYPE_CODE_OPAQUE_POINTER. The
      // PointerRewriter wraps typed uses in TypedPointerType bitcasts so the
      // element type is preserved where it matters; any remaining bare opaque
      // pointer (all PointerTypes are opaque in LLVM 20) is emitted with an
      // opaque (empty struct) element type.
    ...
    case Type::ByteTyID:
      llvmdg::reportError("Byte types are not supported with LLVM 14");
      break;
...
size_t ModuleBitcodeWriter140::addToStrtab(StringRef Str) {
  if (GenerateHash)
    Hasher.update(Str);
  return StrtabBuilder.add(Str);
}

void ModuleBitcodeWriter140::writeComdats() {
  SmallVector<unsigned, 64> Vals;
...
void ModuleBitcodeWriter140::writeValueSymbolTableForwardDecl() {
...
void ModuleBitcodeWriter140::writeModuleInfo() {
...
void ModuleBitcodeWriter140::writeValueAsMetadata(
    const ValueAsMetadata *MD, SmallVectorImpl<uint64_t> &Record) {
  // Mimic an MDNode with a value as one operand.
  Value *V = MD->getValue();
  // In opaque-pointer mode the pointer's element type lives in the PointerMap
  // (enumerated as a TypedPointerType); use it so the emitted type id matches.
  // With typed pointers the value's own (already-enumerated) type is correct and
  // no TypedPointerType was created — don't synthesize one (it would assert in
  // getTypeID, as a metadata-only-referenced function/global is never enumerated
  // as a TypedPointerType).
  Type *Ty = V->getType();
  if (TypedPointerType *PTy = PointerMap.lookup(V))
    Ty = PTy;
  Record.push_back(VE.getTypeID(Ty));
...
void ModuleBitcodeWriter140::writeMDTuple(const MDTuple *N,
                                       SmallVectorImpl<uint64_t> &Record,
                                       unsigned Abbrev) {
...
unsigned ModuleBitcodeWriter140::createDILocationAbbrev() {
...
void ModuleBitcodeWriter140::writeDILocation(const DILocation *N,
                                          SmallVectorImpl<uint64_t> &Record,
                                          unsigned &Abbrev) {
  if (!Abbrev)
...
unsigned ModuleBitcodeWriter140::createGenericDINodeAbbrev() {
  // Assume the column is usually under 128, and always output the inlined-at
  // location (it's never more expensive than building an array size 1).
  auto Abbv = std::make_shared<BitCodeAbbrev>();
...
void ModuleBitcodeWriter140::writeGenericDINode(const GenericDINode *N,
                                             SmallVectorImpl<uint64_t> &Record,
                                             unsigned &Abbrev) {
  if (!Abbrev)
...
void ModuleBitcodeWriter140::writeDISubrange(const DISubrange *N,
                                          SmallVectorImpl<uint64_t> &Record,
                                          unsigned Abbrev) {
...
void ModuleBitcodeWriter140::writeDIGenericSubrange(
...
void ModuleBitcodeWriter140::writeDIEnumerator(const DIEnumerator *N,
                                            SmallVectorImpl<uint64_t> &Record,
                                            unsigned Abbrev) {
...
void ModuleBitcodeWriter140::writeDIBasicType(const DIBasicType *N,
                                           SmallVectorImpl<uint64_t> &Record,
                                           unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(N->getTag());
...
void ModuleBitcodeWriter140::writeDIDerivedType(const DIDerivedType *N,
                                             SmallVectorImpl<uint64_t> &Record,
                                             unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(N->getTag());
...
void ModuleBitcodeWriter140::writeDICompositeType(
...
void ModuleBitcodeWriter140::writeDISubroutineType(
...
void ModuleBitcodeWriter140::writeDIFile(const DIFile *N,
                                      SmallVectorImpl<uint64_t> &Record,
                                      unsigned Abbrev) {
  Record.push_back(N->isDistinct());
...
void ModuleBitcodeWriter140::writeDICompileUnit(const DICompileUnit *N,
                                             SmallVectorImpl<uint64_t> &Record,
                                             unsigned Abbrev) {
...
void ModuleBitcodeWriter140::writeDISubprogram(const DISubprogram *N,
                                            SmallVectorImpl<uint64_t> &Record,
                                            unsigned Abbrev) {
...
void ModuleBitcodeWriter140::writeDILexicalBlock(const DILexicalBlock *N,
                                              SmallVectorImpl<uint64_t> &Record,
                                              unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(VE.getMetadataOrNullID(N->getScope()));
...
void ModuleBitcodeWriter140::writeDILexicalBlockFile(
...
void ModuleBitcodeWriter140::writeDICommonBlock(const DICommonBlock *N,
                                             SmallVectorImpl<uint64_t> &Record,
                                             unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(VE.getMetadataOrNullID(N->getScope()));
...
void ModuleBitcodeWriter140::writeDINamespace(const DINamespace *N,
                                           SmallVectorImpl<uint64_t> &Record,
                                           unsigned Abbrev) {
...
void ModuleBitcodeWriter140::writeDIMacro(const DIMacro *N,
                                       SmallVectorImpl<uint64_t> &Record,
                                       unsigned Abbrev) {
  Record.push_back(N->isDistinct());
...
void ModuleBitcodeWriter140::writeDIMacroFile(const DIMacroFile *N,
                                           SmallVectorImpl<uint64_t> &Record,
                                           unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(N->getMacinfoType());
...
void ModuleBitcodeWriter140::writeDIArgList(const DIArgList *N,
                                         SmallVectorImpl<uint64_t> &Record,
                                         unsigned Abbrev) {
...
void ModuleBitcodeWriter140::writeDIAssignID(const DIAssignID *N,
                                          SmallVectorImpl<uint64_t> &Record,
                                          unsigned Abbrev) {
...
void ModuleBitcodeWriter140::writeDITemplateTypeParameter(
...
void ModuleBitcodeWriter140::writeDITemplateValueParameter(
...
void ModuleBitcodeWriter140::writeDIGlobalVariable(
...
void ModuleBitcodeWriter140::writeDILocalVariable(
...
void ModuleBitcodeWriter140::writeDILabel(
...
void ModuleBitcodeWriter140::writeDIExpression(const DIExpression *N,
                                            SmallVectorImpl<uint64_t> &Record,
                                            unsigned Abbrev) {
...
void ModuleBitcodeWriter140::writeDIGlobalVariableExpression(
...
void ModuleBitcodeWriter140::writeDIObjCProperty(const DIObjCProperty *N,
                                              SmallVectorImpl<uint64_t> &Record,
                                              unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(VE.getMetadataOrNullID(N->getRawName()));
...
void ModuleBitcodeWriter140::writeDIImportedEntity(
...
unsigned ModuleBitcodeWriter140::createNamedMetadataAbbrev() {
  auto Abbv = std::make_shared<BitCodeAbbrev>();
...
void ModuleBitcodeWriter140::writeNamedMetadata(
    SmallVectorImpl<uint64_t> &Record) {
...
unsigned ModuleBitcodeWriter140::createMetadataStringsAbbrev() {
  auto Abbv = std::make_shared<BitCodeAbbrev>();
...
void ModuleBitcodeWriter140::writeMetadataStrings(
...
void ModuleBitcodeWriter140::writeMetadataRecords(
...
void ModuleBitcodeWriter140::writeModuleMetadata() {
...
void ModuleBitcodeWriter140::writeFunctionMetadata(const Function &F) {
  if (!VE.hasMDs())
    return;
...
void ModuleBitcodeWriter140::pushGlobalMetadataAttachment(
...
void ModuleBitcodeWriter140::writeFunctionMetadataAttachment(const Function &F) {
...
void ModuleBitcodeWriter140::writeModuleMetadataKinds() {
  SmallVector<uint64_t, 64> Record;
...
void ModuleBitcodeWriter140::writeOperandBundleTags() {
  // Write metadata kinds
  //
...
void ModuleBitcodeWriter140::writeSyncScopeNames() {
...
void ModuleBitcodeWriter140::writeConstants(unsigned FirstVal, unsigned LastVal,
                                         bool isGlobal) {
  ...
  const ValueEnumerator140::ValueList &Vals = VE.getValues();
  Type *LastTy = nullptr;
...
void ModuleBitcodeWriter140::writeModuleConstants() {
  const ValueEnumerator140::ValueList &Vals = VE.getValues();
...
bool ModuleBitcodeWriter140::pushValueAndType(const Value *V, unsigned InstID,
                                           SmallVectorImpl<unsigned> &Vals,
                                           Type *Ty) {
...
void ModuleBitcodeWriter140::writeOperandBundles(const CallBase &CS,
                                              unsigned InstID) {
...
void ModuleBitcodeWriter140::pushValue(const Value *V, unsigned InstID,
                                    SmallVectorImpl<unsigned> &Vals) {
  unsigned ValID = VE.getValueID(V);
  Vals.push_back(InstID - ValID);
}

void ModuleBitcodeWriter140::pushValueSigned(const Value *V, unsigned InstID,
                                          SmallVectorImpl<uint64_t> &Vals) {
  unsigned ValID = VE.getValueID(V);
...
void ModuleBitcodeWriter140::writeInstruction(const Instruction &I,
                                           unsigned InstID,
                                           SmallVectorImpl<unsigned> &Vals) {
  unsigned Code = 0;
  unsigned AbbrevToUse = 0;
...
void ModuleBitcodeWriter140::writeGlobalValueSymbolTable(
  DenseMap<const Function *, uint64_t> &FunctionToBitcodeIndex) {
...
void ModuleBitcodeWriter140::writeFunctionLevelValueSymbolTable(
...
void ModuleBitcodeWriter140::writeUseList(UseListOrder &&Order) {
...
void ModuleBitcodeWriter140::writeUseListBlock(const Function *F) {
...
void ModuleBitcodeWriter140::writeFunction(
    const Function &F,
    DenseMap<const Function *, uint64_t> &FunctionToBitcodeIndex) {
...
void ModuleBitcodeWriter140::writeBlockInfo() {
...
void IndexBitcodeWriter140::writeModStrings() {
...
void ModuleBitcodeWriterBase140::writePerModuleFunctionSummaryRecord(
...
void ModuleBitcodeWriterBase140::writeModuleLevelReferences(
...
void ModuleBitcodeWriterBase140::writePerModuleGlobalValueSummary() {
  ...
  for (auto &S : Index->typeIdCompatibleVtableMap()) {
    writeTypeIdCompatibleVtableSummaryRecord(NameVals, StrtabBuilder,
                                             std::string(S.first), S.second, VE);
...
void IndexBitcodeWriter140::writeCombinedGlobalValueSummary() {
  ...
  for (auto &T : ReferencedTypeIds) {
    ...
    for (auto It = TidIter.first; It != TidIter.second; ++It) {
      writeTypeIdSummaryRecord(NameVals, StrtabBuilder, std::string(It->second.first),
                               It->second.second);
...
void ModuleBitcodeWriter140::writeModuleHash(size_t BlockStartPos) {
...
void ModuleBitcodeWriter140::write() {
...
BitcodeWriter140::BitcodeWriter140(SmallVectorImpl<char> &Buffer, raw_fd_stream *FS)
  ...
  // parameter is retained for source compatibility but unused.
  (void)FS;
  writeBitcodeHeader(*Stream);
}

BitcodeWriter140::~BitcodeWriter140() { assert(WroteStrtab); }

void BitcodeWriter140::writeBlob(unsigned Block, unsigned Record, StringRef Blob) {
...
void BitcodeWriter140::writeSymtab() {
...
void BitcodeWriter140::writeStrtab() {
  assert(!WroteStrtab);
...
void BitcodeWriter140::copyStrtab(StringRef Strtab) {
  writeBlob(bitc::STRTAB_BLOCK_ID, bitc::STRTAB_BLOB, Strtab);
  WroteStrtab = true;
}

void BitcodeWriter140::writeModule(const Module &M,
                                bool ShouldPreserveUseListOrder,
                                const ModuleSummaryIndex *Index,
  ...
  ModuleBitcodeWriter140 ModuleWriter(M, Buffer, StrtabBuilder, *Stream,
                                   ShouldPreserveUseListOrder, Index,
                                   GenerateHash, ModHash);
  ModuleWriter.write();
}

void BitcodeWriter140::writeIndex(
    const ModuleSummaryIndex *Index,
    const std::map<std::string, GVSummaryMapTy> *ModuleToSummariesForIndex) {
  IndexBitcodeWriter140 IndexWriter(*Stream, StrtabBuilder, *Index,
                                 ModuleToSummariesForIndex);
  IndexWriter.write();
}

/// Write the specified module to the specified output stream.
void llvm::WriteBitcode140ToFile(const Module &M, raw_ostream &Out,
                              bool ShouldPreserveUseListOrder,
                              const ModuleSummaryIndex *Index,
                              bool GenerateHash, ModuleHash *ModHash) {
  ...
  BitcodeWriter140 Writer(Buffer, dyn_cast<raw_fd_stream>(&Out));
...
void IndexBitcodeWriter140::write() {
  Stream.EnterSubblock(bitc::MODULE_BLOCK_ID, 3);
...
void llvm::WriteIndex140ToFile(
    const ModuleSummaryIndex &Index, raw_ostream &Out,
    const std::map<std::string, GVSummaryMapTy> *ModuleToSummariesForIndex) {
  SmallVector<char, 0> Buffer;
  Buffer.reserve(256 * 1024);

  BitcodeWriter140 Writer(Buffer);
...
class ThinLinkBitcodeWriter : public ModuleBitcodeWriterBase140 {
...
void BitcodeWriter140::writeThinLinkBitcode(const Module &M,
                                         const ModuleSummaryIndex &Index,
                                         const ModuleHash &ModHash) {
  assert(!WroteStrtab);
