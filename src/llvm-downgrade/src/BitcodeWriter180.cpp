//===- Bitcode/Writer/BitcodeWriter180.cpp - Bitcode Writer ------------------===//
//
...
public:
  /// Constructs a BitcodeWriterBase180 object that writes to the provided
  /// \p Stream.
  BitcodeWriterBase180(BitstreamWriter &Stream, StringTableBuilder &StrtabBuilder)
...
void BitcodeWriterBase180::writeModuleVersion() {
  // VERSION: [version#]
...
/// ModuleBitcodeWriter180 and ThinLinkBitcodeWriter.
class ModuleBitcodeWriterBase180 : public BitcodeWriterBase180 {
protected:
  /// The Module to write to bitcode.
  const Module &M;

  /// Enumerates ids for all values in the module.
  ValueEnumerator180 VE;
...
public:
  /// Constructs a ModuleBitcodeWriterBase180 object for the given Module,
  /// writing to the provided \p Buffer.
  ModuleBitcodeWriterBase180(const Module &M, StringTableBuilder &StrtabBuilder,
                          BitstreamWriter &Stream,
                          bool ShouldPreserveUseListOrder,
                          const ModuleSummaryIndex *Index)
      : BitcodeWriterBase180(Stream, StrtabBuilder), M(M),
        VE(M, ShouldPreserveUseListOrder), Index(Index) {
    // Assign ValueIds to any callee values in the index that came from
    // indirect call profiles and were recorded as a GUID not a Value*
    // (which would have been assigned an ID by the ValueEnumerator180).
    // The starting ValueId is just after the number of values in the
    // ValueEnumerator180, so that they can be emitted in the VST.
...
class ModuleBitcodeWriter180 : public ModuleBitcodeWriterBase180 {
...
public:
  /// Constructs a ModuleBitcodeWriter180 object for the given Module,
  /// writing to the provided \p Buffer.
  ModuleBitcodeWriter180(const Module &M, SmallVectorImpl<char> &Buffer,
                      StringTableBuilder &StrtabBuilder,
                      BitstreamWriter &Stream, bool ShouldPreserveUseListOrder,
                      const ModuleSummaryIndex *Index, bool GenerateHash,
                      ModuleHash *ModHash = nullptr)
      : ModuleBitcodeWriterBase180(M, StrtabBuilder, Stream,
...
class IndexBitcodeWriter180 : public BitcodeWriterBase180 {
...
public:
  /// Constructs a IndexBitcodeWriter180 object for the given combined index,
  /// writing to the provided \p Buffer. When writing a subset of the index
  /// for a distributed backend, provide a \p ModuleToSummariesForIndex map.
  IndexBitcodeWriter180(BitstreamWriter &Stream, StringTableBuilder &StrtabBuilder,
                     const ModuleSummaryIndex &Index,
                     const std::map<std::string, GVSummaryMapTy>
                         *ModuleToSummariesForIndex = nullptr)
      : BitcodeWriterBase180(Stream, StrtabBuilder), Index(Index),
...
void ModuleBitcodeWriter180::writeAttributeGroupTable() {
  const std::vector<ValueEnumerator180::IndexAndAttrSet> &AttrGrps =
...
void ModuleBitcodeWriter180::writeAttributeTable() {
...
void ModuleBitcodeWriter180::writeTypeTable() {
  const ValueEnumerator180::TypeList &TypeList = VE.getTypes();
...
size_t ModuleBitcodeWriter180::addToStrtab(StringRef Str) {
  if (GenerateHash)
    Hasher.update(Str);
  return StrtabBuilder.add(Str);
}

void ModuleBitcodeWriter180::writeComdats() {
  SmallVector<unsigned, 64> Vals;
...
void ModuleBitcodeWriter180::writeValueSymbolTableForwardDecl() {
...
void ModuleBitcodeWriter180::writeModuleInfo() {
...
void ModuleBitcodeWriter180::writeValueAsMetadata(
...
void ModuleBitcodeWriter180::writeMDTuple(const MDTuple *N,
                                       SmallVectorImpl<uint64_t> &Record,
                                       unsigned Abbrev) {
...
unsigned ModuleBitcodeWriter180::createDILocationAbbrev() {
...
void ModuleBitcodeWriter180::writeDILocation(const DILocation *N,
                                          SmallVectorImpl<uint64_t> &Record,
                                          unsigned &Abbrev) {
  if (!Abbrev)
...
unsigned ModuleBitcodeWriter180::createGenericDINodeAbbrev() {
  // Assume the column is usually under 128, and always output the inlined-at
  // location (it's never more expensive than building an array size 1).
  auto Abbv = std::make_shared<BitCodeAbbrev>();
...
void ModuleBitcodeWriter180::writeGenericDINode(const GenericDINode *N,
                                             SmallVectorImpl<uint64_t> &Record,
                                             unsigned &Abbrev) {
  if (!Abbrev)
...
void ModuleBitcodeWriter180::writeDISubrange(const DISubrange *N,
                                          SmallVectorImpl<uint64_t> &Record,
                                          unsigned Abbrev) {
...
void ModuleBitcodeWriter180::writeDIGenericSubrange(
...
void ModuleBitcodeWriter180::writeDIEnumerator(const DIEnumerator *N,
                                            SmallVectorImpl<uint64_t> &Record,
                                            unsigned Abbrev) {
...
void ModuleBitcodeWriter180::writeDIBasicType(const DIBasicType *N,
                                           SmallVectorImpl<uint64_t> &Record,
                                           unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(N->getTag());
...
void ModuleBitcodeWriter180::writeDIDerivedType(const DIDerivedType *N,
                                             SmallVectorImpl<uint64_t> &Record,
                                             unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(N->getTag());
...
void ModuleBitcodeWriter180::writeDICompositeType(
...
void ModuleBitcodeWriter180::writeDISubroutineType(
...
void ModuleBitcodeWriter180::writeDIFile(const DIFile *N,
                                      SmallVectorImpl<uint64_t> &Record,
                                      unsigned Abbrev) {
  Record.push_back(N->isDistinct());
...
void ModuleBitcodeWriter180::writeDICompileUnit(const DICompileUnit *N,
                                             SmallVectorImpl<uint64_t> &Record,
                                             unsigned Abbrev) {
...
void ModuleBitcodeWriter180::writeDISubprogram(const DISubprogram *N,
                                            SmallVectorImpl<uint64_t> &Record,
                                            unsigned Abbrev) {
...
void ModuleBitcodeWriter180::writeDILexicalBlock(const DILexicalBlock *N,
                                              SmallVectorImpl<uint64_t> &Record,
                                              unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(VE.getMetadataOrNullID(N->getScope()));
...
void ModuleBitcodeWriter180::writeDILexicalBlockFile(
...
void ModuleBitcodeWriter180::writeDICommonBlock(const DICommonBlock *N,
                                             SmallVectorImpl<uint64_t> &Record,
                                             unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(VE.getMetadataOrNullID(N->getScope()));
...
void ModuleBitcodeWriter180::writeDINamespace(const DINamespace *N,
                                           SmallVectorImpl<uint64_t> &Record,
                                           unsigned Abbrev) {
...
void ModuleBitcodeWriter180::writeDIMacro(const DIMacro *N,
                                       SmallVectorImpl<uint64_t> &Record,
                                       unsigned Abbrev) {
  Record.push_back(N->isDistinct());
...
void ModuleBitcodeWriter180::writeDIMacroFile(const DIMacroFile *N,
                                           SmallVectorImpl<uint64_t> &Record,
                                           unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(N->getMacinfoType());
...
void ModuleBitcodeWriter180::writeDIArgList(const DIArgList *N,
                                         SmallVectorImpl<uint64_t> &Record) {
...
void ModuleBitcodeWriter180::writeDIModule(const DIModule *N,
                                        SmallVectorImpl<uint64_t> &Record,
                                        unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  for (auto &I : N->operands())
...
void ModuleBitcodeWriter180::writeDIAssignID(const DIAssignID *N,
                                          SmallVectorImpl<uint64_t> &Record,
                                          unsigned Abbrev) {
...
void ModuleBitcodeWriter180::writeDITemplateTypeParameter(
...
void ModuleBitcodeWriter180::writeDITemplateValueParameter(
...
void ModuleBitcodeWriter180::writeDIGlobalVariable(
...
void ModuleBitcodeWriter180::writeDILocalVariable(
...
void ModuleBitcodeWriter180::writeDILabel(
...
void ModuleBitcodeWriter180::writeDIExpression(const DIExpression *N,
                                            SmallVectorImpl<uint64_t> &Record,
                                            unsigned Abbrev) {
...
void ModuleBitcodeWriter180::writeDIGlobalVariableExpression(
...
void ModuleBitcodeWriter180::writeDIObjCProperty(const DIObjCProperty *N,
                                              SmallVectorImpl<uint64_t> &Record,
                                              unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(VE.getMetadataOrNullID(N->getRawName()));
...
void ModuleBitcodeWriter180::writeDIImportedEntity(
...
unsigned ModuleBitcodeWriter180::createNamedMetadataAbbrev() {
  auto Abbv = std::make_shared<BitCodeAbbrev>();
...
void ModuleBitcodeWriter180::writeNamedMetadata(
    SmallVectorImpl<uint64_t> &Record) {
...
unsigned ModuleBitcodeWriter180::createMetadataStringsAbbrev() {
  auto Abbv = std::make_shared<BitCodeAbbrev>();
...
void ModuleBitcodeWriter180::writeMetadataStrings(
...
void ModuleBitcodeWriter180::writeMetadataRecords(
...
void ModuleBitcodeWriter180::writeModuleMetadata() {
...
void ModuleBitcodeWriter180::writeFunctionMetadata(const Function &F) {
  if (!VE.hasMDs())
    return;
...
void ModuleBitcodeWriter180::pushGlobalMetadataAttachment(
...
void ModuleBitcodeWriter180::writeFunctionMetadataAttachment(const Function &F) {
...
void ModuleBitcodeWriter180::writeModuleMetadataKinds() {
  SmallVector<uint64_t, 64> Record;
...
void ModuleBitcodeWriter180::writeOperandBundleTags() {
  // Write metadata kinds
  //
...
void ModuleBitcodeWriter180::writeSyncScopeNames() {
...
void ModuleBitcodeWriter180::writeConstants(unsigned FirstVal, unsigned LastVal,
                                         bool isGlobal) {
  ...
  const ValueEnumerator180::ValueList &Vals = VE.getValues();
  Type *LastTy = nullptr;
...
void ModuleBitcodeWriter180::writeModuleConstants() {
  const ValueEnumerator180::ValueList &Vals = VE.getValues();
...
bool ModuleBitcodeWriter180::pushValueAndType(const Value *V, unsigned InstID,
                                           SmallVectorImpl<unsigned> &Vals) {
...
void ModuleBitcodeWriter180::writeOperandBundles(const CallBase &CS,
                                              unsigned InstID) {
...
void ModuleBitcodeWriter180::pushValue(const Value *V, unsigned InstID,
                                    SmallVectorImpl<unsigned> &Vals) {
  unsigned ValID = VE.getValueID(V);
  Vals.push_back(InstID - ValID);
}

void ModuleBitcodeWriter180::pushValueSigned(const Value *V, unsigned InstID,
                                          SmallVectorImpl<uint64_t> &Vals) {
  unsigned ValID = VE.getValueID(V);
...
void ModuleBitcodeWriter180::writeInstruction(const Instruction &I,
                                           unsigned InstID,
                                           SmallVectorImpl<unsigned> &Vals) {
  unsigned Code = 0;
  unsigned AbbrevToUse = 0;
...
void ModuleBitcodeWriter180::writeGlobalValueSymbolTable(
  DenseMap<const Function *, uint64_t> &FunctionToBitcodeIndex) {
...
void ModuleBitcodeWriter180::writeFunctionLevelValueSymbolTable(
...
void ModuleBitcodeWriter180::writeUseList(UseListOrder &&Order) {
...
void ModuleBitcodeWriter180::writeUseListBlock(const Function *F) {
...
void ModuleBitcodeWriter180::writeFunction(
    const Function &F,
    DenseMap<const Function *, uint64_t> &FunctionToBitcodeIndex) {
...
void ModuleBitcodeWriter180::writeBlockInfo() {
...
void IndexBitcodeWriter180::writeModStrings() {
...
void ModuleBitcodeWriterBase180::writeModuleLevelReferences(
...
void ModuleBitcodeWriterBase180::writePerModuleGlobalValueSummary() {
...
void IndexBitcodeWriter180::writeCombinedGlobalValueSummary() {
...
void ModuleBitcodeWriter180::writeModuleHash(size_t BlockStartPos) {
...
void ModuleBitcodeWriter180::write() {
...
BitcodeWriter180::BitcodeWriter180(SmallVectorImpl<char> &Buffer, raw_fd_stream *FS)
...
BitcodeWriter180::~BitcodeWriter180() { assert(WroteStrtab); }

void BitcodeWriter180::writeBlob(unsigned Block, unsigned Record, StringRef Blob) {
...
void BitcodeWriter180::writeSymtab() {
...
void BitcodeWriter180::writeStrtab() {
  assert(!WroteStrtab);
...
void BitcodeWriter180::copyStrtab(StringRef Strtab) {
  writeBlob(bitc::STRTAB_BLOCK_ID, bitc::STRTAB_BLOB, Strtab);
  WroteStrtab = true;
}

void BitcodeWriter180::writeModule(const Module &M,
                                bool ShouldPreserveUseListOrder,
                                const ModuleSummaryIndex *Index,
  ...
  ModuleBitcodeWriter180 ModuleWriter(M, Buffer, StrtabBuilder, *Stream,
                                   ShouldPreserveUseListOrder, Index,
                                   GenerateHash, ModHash);
  ModuleWriter.write();
}

void BitcodeWriter180::writeIndex(
    const ModuleSummaryIndex *Index,
    const std::map<std::string, GVSummaryMapTy> *ModuleToSummariesForIndex) {
  IndexBitcodeWriter180 IndexWriter(*Stream, StrtabBuilder, *Index,
                                 ModuleToSummariesForIndex);
  IndexWriter.write();
}

/// Write the specified module to the specified output stream.
void llvm::WriteBitcode180ToFile(const Module &M, raw_ostream &Out,
                              bool ShouldPreserveUseListOrder,
                              const ModuleSummaryIndex *Index,
                              bool GenerateHash, ModuleHash *ModHash) {
  ...
  BitcodeWriter180 Writer(Buffer, dyn_cast<raw_fd_stream>(&Out));
...
void IndexBitcodeWriter180::write() {
  Stream.EnterSubblock(bitc::MODULE_BLOCK_ID, 3);
...
void llvm::WriteIndex180ToFile(
    const ModuleSummaryIndex &Index, raw_ostream &Out,
    const std::map<std::string, GVSummaryMapTy> *ModuleToSummariesForIndex) {
  SmallVector<char, 0> Buffer;
  Buffer.reserve(256 * 1024);

  BitcodeWriter180 Writer(Buffer);
...
class ThinLinkBitcodeWriter : public ModuleBitcodeWriterBase180 {
...
void BitcodeWriter180::writeThinLinkBitcode(const Module &M,
                                         const ModuleSummaryIndex &Index,
                                         const ModuleHash &ModHash) {
  assert(!WroteStrtab);
