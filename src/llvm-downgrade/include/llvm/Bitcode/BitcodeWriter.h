...
public:
  ...
  public:
    /// Create a BitcodeWriter70 that writes to Buffer.
    BitcodeWriter70(SmallVectorImpl<char> &Buffer);
  ...
  public:
    /// Create a BitcodeWriter180 that writes to Buffer.
    BitcodeWriter180(SmallVectorImpl<char> &Buffer, raw_fd_stream *FS = nullptr);
...
LLVM_ABI void WriteBitcodeToFile(const Module &M, raw_ostream &Out,
  ...
  void WriteBitcode70ToFile(const Module &M, raw_ostream &Out,
                            bool ShouldPreserveUseListOrder = false,
                            const ModuleSummaryIndex *Index = nullptr,
                            bool GenerateHash = false,
                            ModuleHash *ModHash = nullptr);
