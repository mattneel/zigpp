...
LLVM_BIN=${LLVM_BIN:-/home/autark/src/zig/zigpp-bootstrap/out/build-llvm-host/bin}
AIR_REWRITE=${AIR_REWRITE:-$here/../../../../air-rewrite-build/air-rewrite}
DOWNGRADE=${DOWNGRADE:-/tmp/metal-spike/downgrade/build/llvm-downgrade}
METALLIB=${METALLIB:-$here/metallib}
...
echo "== 4. bitcode -> .metallib (one library per kernel, same module bytes)"
for k in vadd reduce parsef; do
    "$METALLIB" write --bitcode "$out/kernels.air14.bc" --name "$k" --air 2.8 --metal 4.0 \
        --format 1.2.9 --platform 26.0.0 --uuid -o "$out/$k.metallib"
done
